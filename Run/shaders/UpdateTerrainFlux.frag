...
{ 
	...
	ivec2 tsize = textureSize(s_heightData, 0);
	if ( texelCoordL.x < 0 )
	{
		rockHeightN.x = rockHeightC.x;
		moltenHeightN.x = moltenHeightC.x;
		dirtHeightN.x = dirtHeightC.x;
		waterHeightN.x = waterHeightC.x;
	}
	if ( texelCoordR.x >= tsize.x )
	{
		rockHeightN.y = rockHeightC.y;
		moltenHeightN.y = moltenHeightC.y;
		dirtHeightN.y = dirtHeightC.y;
		waterHeightN.y = waterHeightC.y;
	}
	if ( texelCoordU.y < 0 )
	{
		rockHeightN.z = rockHeightC.z;
		moltenHeightN.z = moltenHeightC.z;
		dirtHeightN.z = dirtHeightC.z;
		waterHeightN.z = waterHeightC.z;
	}
	if ( texelCoordD.y >= tsize.y )
	{
		rockHeightN.w = rockHeightC.w;
		moltenHeightN.w = moltenHeightC.w;
		dirtHeightN.w = dirtHeightC.w;
		waterHeightN.w = waterHeightC.w;
	}
