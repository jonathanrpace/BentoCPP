...
template<typename T>
void bento::NodeGroup<T>::AddIfMatch(Entity entity)
{
	...
	for (size_t i = 0; i < nodeInfo.GetMembersCount(); i++)
	{
		...
		for (ComponentPtr componentPtr : componentsForThisEntity)
		{
			if (componentPtr->typeInfo() == typeInfo)
			{
				...
				bento::IComponent* nakedPointer = componentPtr.get();
				*(bento::IComponent**)ptr = nakedPointer;
...
template<typename T>
void bento::NodeGroup<T>::RemoveIfMatch(Entity entity)
{
...
template<typename T>
void bento::NodeGroup<T>::OnEntityRemoved(Entity entity)
{
	RemoveIfMatch(entity);
}

template<typename T>
void bento::NodeGroup<T>::OnComponentAddedToEntity(Entity entity, ComponentPtr component)
{
	AddIfMatch(entity);
}

template<typename T>
void bento::NodeGroup<T>::OnComponentRemovedFromEntity(Entity entity, ComponentPtr component)
{
	RemoveIfMatch(entity);
}
