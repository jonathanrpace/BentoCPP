	...
	)
{
	...
	size_t resolvedFilenameSize = strlen(_filename) + bento::Config::ResourcePath().size() + 1;
	char* resolvedFilename = new char[resolvedFilenameSize];
	strcpy_s(resolvedFilename, resolvedFilenameSize, bento::Config::ResourcePath().data());
