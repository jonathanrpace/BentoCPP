...
namespace bento
{
	...
	void SteamParticleGeom::Validate()
	{
		...
		for (int i = 0; i < m_numParticles; i++)
		{
			...
			properties[float4Index + 2] = Rand();
			properties[float4Index + 3] = Rand();
		}
		...
		// Transfer the data to the buffers, and bind them together, associating some with transform feedback
		{
			...
			GL_CHECK(glBindBufferBase(GL_TRANSFORM_FEEDBACK_BUFFER, 0, m_positionBufferA));									// Position will be fed back to 0 index buffer.
